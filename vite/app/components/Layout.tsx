  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0 pl-[10px] pr-[10px]">
        {/* Header */}
        <header className="sticky top-0 z-50 backdrop-blur-[2px] px-0 md:px-0 pt-[20px] pb-2 md:py-4">
          <div className="max-w-[1632px] mx-auto flex items-center justify-center md:justify-between gap-3 md:gap-8 relative mx-[0px] mt-[8px] mb-[0px]">
            {/* Filter Tabs - Centered on mobile, accounting for sidebar */}
            ...
            {isLoggedIn && (
              <div className="absolute right-0 top-0 md:relative flex items-center gap-2 md:gap-3 bg-gradient-to-br from-[#d5cab8] to-[#c9bfad] px-2 md:px-4 py-1.5 md:py-2 rounded-xl shadow-md h-[32px] md:h-[40px]">
                ...
                <span className="font-medium text-xs md:text-sm text-[#3A3630]">{loginName}</span>
                <button
                  onClick={handleSignOut}
                  className="ml-1 md:ml-2 text-[10px] md:text-xs text-[#3A3630] hover:underline"
                >
                  Sign Out
                </button>
              </div>
