...
import { VisionaryCircleSidebar } from "@/app/components/VisionaryCircleSidebar";
import svgBrandMark from "@/imports/svg-dbcm4od5aw";

type FilterType = "All" | "Read" | "Listen" | "Watch";

export function VisionaryCircleLayout() {
  ...
  // Join Visionary Circle form fields
  ...
  // Step tracking
  const [joinKyozoStep, setJoinKyozoStep] = useState(1);
  const [joinCommunityStep, setJoinCommunityStep] = useState(1);
  ...
  const showFilterTabs = location.pathname === "/visionary-circle" || location.pathname === "/visionary-circle/feed";

  return (
    <div className="relative min-h-screen bg-[#7a7a7a] flex">
      {/* Visionary Circle Sidebar */}
      <VisionaryCircleSidebar 
      ...
      <div className="flex-1 min-w-0">
        {/* Background overlay - darker grey theme */}
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute bg-[rgba(90,90,90,0.4)] inset-0 mix-blend-overlay" />
        </div>
        ...
        <header className="sticky top-0 z-50 backdrop-blur-[2px] px-2 md:px-12 pt-[13px] pb-2 md:py-4">
          <div className="max-w-[1632px] mx-auto flex items-center justify-center md:justify-between gap-3 md:gap-8 relative">
            ...
            {isLoggedIn && (
              <div className="hidden md:flex items-center gap-2 md:gap-3 bg-gradient-to-br from-[#4a4a4a] to-[#3a3a3a] px-2 md:px-4 py-1.5 md:py-2 rounded-xl shadow-md h-[32px] md:h-[40px]">
                <div className="size-6 md:size-8 rounded-full bg-[#7c6f84] flex items-center justify-center text-white font-bold text-xs md:text-sm">
                  {loginName.charAt(0).toUpperCase()}
                </div>
                <span className="font-medium text-xs md:text-sm text-[#e0e0e0] hidden sm:inline">{loginName}</span>
                <button
                  onClick={handleSignOut}
                  className="ml-1 md:ml-2 text-[10px] md:text-xs text-[#b0b0b0] hover:underline hidden sm:inline"
                >
                  Sign Out
                </button>
              </div>
        ...
        {/* Join Kyozo Modal - Same as Willer Universe */}
        {showJoinKyozoModal && (
          <div className="fixed inset-0 bg-black/30 z-[100] flex items-center justify-center md:p-4">
        ...
        {/* Join Visionary Circle Community Modal */}
            ...
            <div className="bg-gradient-to-br from-[#f8f7fc] to-[#fefefe] w-full h-full md:h-auto md:rounded-3xl shadow-2xl md:max-w-lg p-6 md:p-10 animate-in fade-in md:zoom-in-95 duration-200 overflow-y-auto relative">
              ...
              {joinCommunityStep === 1 && (
                <>
                  ...
                  <div className="flex items-start justify-between mb-8 pr-8">
                    <div className="flex items-start gap-3 md:gap-4">
                      <div className="relative shrink-0 size-12 md:size-14 mt-1 bg-[#7c6f84] rounded-full flex items-center justify-center border-2 border-[#9b8ea5]">
                        <span className="text-xl font-bold text-white">VC</span>
                      </div>
                      <div>
                        <h2 className="text-2xl md:text-4xl font-bold text-[#111827] mb-2 md:mb-3">Join Visionary Circle</h2>
                        <p className="text-sm md:text-base text-[#6b7280]">The Kyozo Investor Pool</p>
                      </div>
                    </div>
                  </div>
              ...
              {joinCommunityStep === 3 && (
                <div className="flex flex-col items-center justify-center py-8 md:py-12">
                  ...
                  <h2 className="text-3xl md:text-4xl font-bold text-[#111827] mb-3 text-center">Welcome to Visionary Circle!</h2>
                  <p className="text-base md:text-lg text-[#6b7280] text-center">
                    You've successfully joined the private community
                  </p>
                </div>
              )}
            </div>
